...
// Builds on 04_so_compute_attainment.js with two enhancements:
//   - Multiple metrics per course: assessment_type = ANY(chosen_metrics)
//   - Per-course threshold:        COALESCE(c.threshold, $global_threshold)
//
// Primary path:  assessment_scores × course_faculty_config
// Fallback path: student_grades × course_faculty_config (PASS_GRADES set)
...
const globalThreshold = Number(query.threshold || 70);

if (!term) {
  return [{ json: { status: 'error', message: 'term query parameter is required' } }];
}
if (globalThreshold < 1 || globalThreshold > 100) {
...
try {
  // Primary path: assessment-level scores.
  // JOIN on ANY(chosen_metrics) so multiple metric types are included.
  // COALESCE(c.threshold, $2) uses per-course threshold, falling back to global.
  const assessmentResult = await client.query(
    `WITH student_avg AS (
       SELECT
         ...
         c.chosen_s,
         c.chosen_metrics,
         c.threshold          AS course_threshold,
         s.student_id,
       ...
       INNER JOIN course_faculty_config c
         ...
         AND s.assessment_type = ANY(c.chosen_metrics)
       WHERE s.term = $1
       GROUP BY s.program, s.course_code, c.chosen_s, c.chosen_metrics, c.threshold, s.student_id
     )
     SELECT
       faculty,
       course_code,
       ...
       chosen_metrics,
       course_threshold,
       COALESCE(course_threshold, $2)                                              AS effective_threshold,
       COUNT(*)::int                                                                AS students_assessed,
       COUNT(CASE WHEN student_score >= COALESCE(course_threshold, $2) THEN 1 END)::int
                                                                                   AS students_attained,
       ROUND(AVG(student_score)::numeric, 1)::float                                AS average_score,
       ROUND(
         COUNT(CASE WHEN student_score >= COALESCE(course_threshold, $2) THEN 1 END)::numeric
         / NULLIF(COUNT(*), 0)::numeric * 100,
       1)::float                                                                    AS attainment_rate
     FROM student_avg
     GROUP BY faculty, course_code, chosen_s, chosen_metrics, course_threshold
     ORDER BY faculty, course_code`,
    [term, globalThreshold]
  );

  let courseRows = assessmentResult.rows;

  // Fallback path: final grades only (PASS_GRADES set).
  if (!courseRows.length) {
    const fallbackResult = await client.query(
      `SELECT
         ...
         c.chosen_metrics,
         c.threshold              AS course_threshold,
         COALESCE(c.threshold, $2) AS effective_threshold,
         COUNT(*)::int             AS students_assessed,
         COUNT(CASE WHEN UPPER(g.grade) = ANY(ARRAY['A+','A','A-','B+','B','B-','C+','C'])
               THEN 1 END)::int   AS students_attained,
       ...
       WHERE g.term = $1
       GROUP BY g.program, g.course_code, c.chosen_s, c.chosen_metrics, c.threshold
  ...
  // Group by faculty (mirrors soGroups in 04_so_compute_attainment.js)
  const facultyMap = new Map();
  for (const row of courseRows) {
    const faculty = String(row.faculty);
    if (!facultyMap.has(faculty)) facultyMap.set(faculty, { faculty, courses: [] });

    const effectiveThreshold = Number(row.effective_threshold || globalThreshold);
    const attainmentRate = Number(row.attainment_rate || 0);
    const averageScore = row.average_score !== null ? Number(row.average_score) : null;
    const meetsThreshold = attainmentRate >= effectiveThreshold;

    facultyMap.get(faculty).courses.push({
      course_code:        String(row.course_code),
      chosen_s:           Number(row.chosen_s),
      s_label:            `S${row.chosen_s}`,
      chosen_metrics:     Array.isArray(row.chosen_metrics) ? row.chosen_metrics : ['QUIZ'],
      threshold:          row.course_threshold !== null ? Number(row.course_threshold) : null,
